...
def create_data_part():
    train_csv_path = os.path.join(cfg.label_path, 'train.csv')
    val_csv_path = os.path.join(cfg.label_path, 'val.csv')
    test_csv_path = os.path.join(cfg.label_path, 'test.csv')

    train_ds = AVADataset_text(train_csv_path, cfg.text_csv_path,
                          cfg.image_path, if_train=True)
    val_ds = AVADataset_text(val_csv_path, cfg.text_csv_path,
...
def train(model, discriminator, loader, optimizer, optimizer_d, criterion, criterion2):
    model.train()
    discriminator.train()
    ...
    for idx, (x, t, y) in enumerate(tqdm(loader)):
        ...
        real_labels = torch.ones(cfg.batch_size, 1).to(device)
        fake_labels = torch.zeros(cfg.batch_size, 1).to(device)

        #  Train Discriminator
        optimizer_d.zero_grad()
        fake_pred = model(x, t).float()
        real = discriminator(y)
        fake = discriminator(fake_pred.detach())
        real_loss = criterion2(real, real_labels)
        fake_loss = criterion2(fake, fake_labels)
        ...
        #  Train Generator
        optimizer.zero_grad()
        y_pred = discriminator(fake_pred)
        loss_reg = criterion(fake_pred, y)
        loss_adv = criterion2(y_pred, real_labels)
        g_loss = loss_reg + loss_adv
        g_loss.backward()
        optimizer.step()

        train_losses.update(g_loss.item(), x.size(0))
    return train_losses.avg


def validate(model, loader, criterion, test_or_valid_flag='valid'):
    model.eval()
...
def start_train():
    dataloader_train, dataloader_valid, dataloader_test = create_data_part()
    criterion = EDMLoss().to(device)
    criterion2 = nn.BCELoss().to(device)
    model = MIGFNet().to(device)
    discriminator = Discriminator().to(device)

    optimizer = optim.Adam(model.parameters(), lr=cfg.learning_rate, betas=cfg.adam_betas)
    optimizer_d = optim.Adam(discriminator.parameters(), lr=cfg.learning_rate, betas=cfg.adam_betas)
    ...
    for e in range(cfg.n_epoch):
        ...
        train_loss = train(model=model, discriminator=discriminator, loader=dataloader_train, optimizer=optimizer, optimizer_d=optimizer_d, criterion=criterion, criterion2=criterion2)
        print('train_loss:', train_loss)
        val_loss, vacc, vlcc, vsrcc = validate(model=model, loader=dataloader_valid, criterion=criterion, test_or_valid_flag='valid')
        ...
        if (e + 1) % cfg.save_freq == 0:
            ...
            filename = "epoch_{}_{}_{}_val_loss_{:.4f}_.pt".format(
                e+1, vsrcc, vlcc, val_loss
            )
            torch.save(model.state_dict(), os.path.join(cfg.save_path, filename))
