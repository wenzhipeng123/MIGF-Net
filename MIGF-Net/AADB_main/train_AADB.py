...
def create_data_part():
    # Load datasets
    train_dataset = AADB(
        image_dir=cfg.image_path, text_dir=cfg.text_path, labels_dir=cfg.label_path, split="train",
    )
    val_dataset = AADB(
        image_dir=cfg.image_path, text_dir=cfg.text_path, labels_dir=cfg.label_path, split="val",
    )

    train_loader = DataLoader(train_dataset, batch_size=cfg.batch_size, num_workers=cfg.num_workers, shuffle=True, prefetch_factor=cfg.prefetch_factor, drop_last=True)
    val_loader = DataLoader(val_dataset, batch_size=cfg.batch_size, num_workers=cfg.num_workers, shuffle=False, prefetch_factor=cfg.prefetch_factor)
...
def start_train():
    ...
    dataloader_train, dataloader_val = create_data_part()
    ...
    for e in range(cfg.n_epoch):
        ...
        train_loss = train(model=model, loader=dataloader_train, optimizer=optimizer, criterion=criterion)
        print('train_loss:', train_loss)
        val_loss, vacc, vlcc, vsrcc = validate(model=model, loader=dataloader_val, criterion=criterion)
